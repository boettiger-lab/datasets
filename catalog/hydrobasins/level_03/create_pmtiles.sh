#!/bin/bash
# Create PMTiles from HydroBasins Level 3

set -e

echo "Converting HydroBasins Level 3 to GeoJSONSeq for tippecanoe..."

# Convert parquet to GeoJSONSeq (which tippecanoe can read)
# Use GeoJSONSeq for streaming large files
ogr2ogr \
  -f GeoJSONSeq \
  /tmp/hydrobasins_level_03.geojsonl \
...
tippecanoe \
  -o /tmp/hydrobasins_level_03.pmtiles \
  -l hydrobasins_level_03 \
...
mc cp /tmp/hydrobasins_level_03.pmtiles s3/public-hydrobasins/level_03/hydrobasins_level_03.pmtiles

echo "PMTiles uploaded successfully!"

# Clean up temporary files
rm -f /tmp/hydrobasins_level_03.geojsonl /tmp/hydrobasins_level_03.pmtiles
