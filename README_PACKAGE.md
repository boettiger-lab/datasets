...
# Generate S3 sync job
from cng_datasets.k8s import generate_sync_job

generate_sync_job(
    job_name="sync-to-backup",
...
# Generate K8s job for syncing between S3 locations
cng-datasets sync-job \
    --job-name sync-to-backup \
    --source nrp:public-dataset/data \
    --destination aws:backup-bucket/data \
