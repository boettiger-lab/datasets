...
- **The hex path no longer silently reads band 1 of a multi-band raster.** `cng-datasets raster --output-parquet` passed the source to exactextract with no band index, and exactextract defaults to the first band; `--band` was documented as "mosaic only" and was never consulted when hexing. Nothing in the run, the output schema or the parquet said which band was used, and the value column is named by `--value-column` either way — so the output looked correct and carried a label asserting it was something it was not. This published 384,922,346 rows of annual forb & grass cover documented as perennial cover, two variables in the same 0–100 domain that no value-range or schema check could tell apart (#214). Two changes: hexing a multi-band source with no band selected is now **refused outright** — a job that fails at submission is far cheaper than a dataset that has to be re-measured against the source to be doubted — and `--band` is honoured on every path, not just the mosaic. The band is selected once, at the source, as a single-band VRT view, so nodata detection, the windowed read, the fill-code collapse, exactextract and the warp all see a raster with exactly one band and no later step can pick differently. `--band` on `raster-workflow` is unchanged in behaviour (it adds a preprocess-cog step that subsets the band) but its help no longer says "multi-tile only", which it never was
