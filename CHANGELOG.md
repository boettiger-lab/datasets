...
- `--h0-index` and `--h0-subset` are no longer documented as "h0 base cells". They are **positions** in the h0 grid's own `i` column, which is an arbitrary permutation of the 122 base cells: position 12 is base cell 9, and exactly one of the 122 positions coincides with its own base cell. Both numberings run 0–121, so a base-cell list — which is what you get from the obvious approach of computing coverage with the H3 library — is accepted without complaint and resolves to entirely different cells. The job then succeeds, writes the expected number of partitions, and covers the wrong geography, which no structural check can catch. The mislabel is corrected in the CLI help, the generated manifest comment and the generator docstring; `--h0-cells` above gives the base-cell list a correct path; every pod now logs what its position resolved to in its first lines (`h0 cell: 577199624117288959 (h3 8029fffffffffff, H3 base cell 20)`) so a mis-specified subset is visible at start-up rather than in the finished product's extent; and `docs/raster_processing.md` carries the measured CONUS mapping plus the query that reconstructs it. That last part matters because the mapping was **not** derivable from any published artifact: reading `s3://public-grids/hex/h0-valid.parquet` in row order sorts by cell id and therefore by base cell, giving position == base cell for all 122 rows — a confident, self-consistent, wrong answer. It is the `i` column, not the row order (#213)

## [0.6.0] - 2026-09-13

### Added
- `--armada-queue` on `workflow` and `raster-workflow`. A submitted Armada job set carries `queue` at the top and `namespace` on each job: the queue is Armada's scheduling and accounting entity, the namespace is where the pods are actually created. NRP maps them one-to-one by convention, not by format, and the generators hardwired `queue = namespace` with no way to say otherwise. It still defaults to the namespace (#183)
- `namespace` is now a cluster-profile field, so a site sets it in one place rather than on every command. `--namespace` still overrides it (#201)
- `cng-datasets gapfill`: emits an Armada job set that re-runs exactly the sub-h0 chunks that never completed. Armada exposes no retry service on NRP — `armadactl get retry-policies` returns Unimplemented — and a preempted job is not rescheduled, so a transient fault leaves a permanently missing chunk; at an observed ~0.1% failure rate a few-thousand-unit fan-out loses one or two every run, which makes gap-fill a pipeline stage rather than an exception. Gaps come from the completion markers on S3, so this needs **no Armada and no Kubernetes access** — it runs as an ordinary S3 client after a job set finishes, whatever submitted it; only re-submitting needs `armadactl`. Exits 0 when nothing is missing and 1 when a job set was written, so a pipeline can branch on it the way it would on `diff`. The k8s backend does not need this: `backoffLimitPerIndex` retries a failed index in place, which is precisely the budget conversion cannot carry across (#183, #173)
...
### Changed
- The merge pod's ephemeral storage default drops from 100Gi to **50Gi**, the house limit. `geo-workflows` enforces no cpu, memory or pod quota — only priority-class bans — but `biodiversity` caps pods at 200, and sizing for the stricter namespace keeps a build portable between them. The merge holds one partition at a time and deletes it after upload, so it never needed the larger figure. `tests/.../TestResourceNorms` now asserts 50Gi ephemeral and 200 concurrent pods against the *emitted manifests*, so a future default cannot quietly exceed them. Note the pod limit binds on `parallelism`, not `completions`: a `pods: 200` quota counts pods alive at once, so an 842-completion fan-out at parallelism 61 is fine (#201)
- **Behaviour change:** the default Kubernetes namespace is now `geo-workflows`, not `biodiversity`, in both the `ClusterConfig` default and the built-in `nrp` profile. Generated manifests, the workflow RBAC and — because the Armada queue follows the namespace — the queue a converted job set submits to all move with it. Pass `--namespace biodiversity` to keep the old target

### Fixed
- A partly failed sub-h0 fan-out can no longer be published as a complete dataset. `merge-chunks` consolidated whatever part files it found and then, with cleanup on, purged the chunks prefix — so a build that lost chunks produced a short dataset, exit 0, with the evidence deleted. Counting parts cannot detect this: a chunk that does not overlap the raster legitimately writes none, so a missing part is indistinguishable from a chunk that never ran. Every chunk now records a completion marker **whether or not it wrote data**, and `--expect-chunks N` (emitted automatically by `raster-workflow`, which sized the fan-out) makes the merge refuse to run unless that many chunks completed. The missing chunks are **enumerated, not counted**: a count answers "48 of 49" and leaves you to find the one, which on a few-thousand-unit fan-out is the entire problem, so the error names the indices and `cng-datasets gapfill` re-runs precisely those. Checked before anything is read or written, so a refused merge costs nothing and leaves every chunk in place. The risk was concentrated on the Armada backend, whose converted jobs carry no retry budget (#183) and which is where `--backend auto` routes a large fan-out (#173)
...
- The merge job's `backoffLimit` goes from 0 to 2. It runs after hours of hex work, and a transient S3 failure should not throw that away (#201)
