#!/bin/bash
# Create PMTiles from HydroBasins Level 5

set -e

echo "Converting HydroBasins Level 5 to GeoJSONSeq for tippecanoe..."

# Convert parquet to GeoJSONSeq (which tippecanoe can read)
# Use GeoJSONSeq for streaming large files
ogr2ogr \
  -f GeoJSONSeq \
  /tmp/hydrobasins_level_05.geojsonl \
...
tippecanoe \
  -o /tmp/hydrobasins_level_05.pmtiles \
  -l hydrobasins_level_05 \
...
mc cp /tmp/hydrobasins_level_05.pmtiles s3/public-hydrobasins/level_05/hydrobasins_level_05.pmtiles

echo "PMTiles uploaded successfully!"

# Clean up temporary files
rm -f /tmp/hydrobasins_level_05.geojsonl /tmp/hydrobasins_level_05.pmtiles
