...
def main():
    parser = argparse.ArgumentParser(description="Process HydroBasins level 4 into H3 hexes")
    ...
    parser.add_argument("--input-url", default="s3://public-hydrobasins/level_04.parquet", help="Input geoparquet file")
    parser.add_argument("--output-url", default="s3://public-hydrobasins/level_04/chunks", help="Output geoparquet bucket")
    ...
    # If HYBAS_ID exists, rename it to 'id' for consistency
    if 'HYBAS_ID' in keep_cols:
        table = table.rename(id='HYBAS_ID')
        keep_cols = [c if c != 'HYBAS_ID' else 'id' for c in keep_cols]
