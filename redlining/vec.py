...
def main():
    parser = argparse.ArgumentParser(description="Process redlining polygon file into H3 hexes")
    parser.add_argument("--i", type=int, default=0, help="Chunk index to process (0-based)")
    parser.add_argument("--zoom", type=int, default=10, help="H3 resolution to aggregate to (default 10)")
    parser.add_argument("--chunk-size", type=int, default=500, help="Number of rows per chunk")
    parser.add_argument("--input-url", default="s3://public-redlining/mappinginequality.parquet", help="Input geoparquet file")
    parser.add_argument("--output-url", default="s3://public-redlining/chunks", help="Output geoparquet bucket")
    ...
    result = (
        ...
        .mutate(h10=_.h3id.unnest())
        .mutate(
            h9=h3_cell_to_parent(_.h10, 9),
            h8=h3_cell_to_parent(_.h10, 8),
            h0=h3_cell_to_parent(_.h10, 0)
        )
        .drop('h3id')
    )
