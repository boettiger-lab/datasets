...
We have also pre-processed the polygons in this dataset into [h3 spatial indices](https://h3geo.org) at various zooms. We access zoom level 8 (corresponding to hexagons whose edge lengths are approximately 500 meters, see [resolutions tables](https://h3geo.org/docs/core-library/restable#edge-lengths)). Later we will revisit this step.

```{r}
pad_z8 =  open_dataset("s3://public-biodiversity/pad-us-4/pad-h3-z8.parquet", recursive = FALSE)
```

We can filter this data by various aspects, such looking only at National Park Service (NPS) managed lands.  But what if we only those parks in a particular State or County?  
